...
global function MpWeaponSmart_Car_Init

global function OnWeaponActivate_weapon_smart_car
global function OnWeaponDeactivate_weapon_smart_car
global function OnWeaponPrimaryAttack_weapon_smart_car
global function OnWeaponBulletHit_weapon_smart_car
global function Smart_carPrecache
global function OnWeaponStartZoomIn_weapon_smart_car
global function OnWeaponStartZoomOut_weapon_smart_car

const float SMART_CAR_TRACKER_TIME = 15.0

function MpWeaponSmart_Car_Init()
{
	PrecacheParticleSystem( $"P_smartpistol_lockon_FP" )
	PrecacheParticleSystem( $"P_smartpistol_lockon" )
}
void function Smart_carPrecache()
{
	PrecacheWeapon( "vm_weapon_smart_car" )
}
void function OnWeaponActivate_weapon_smart_car( entity weapon )
{
...
void function OnWeaponDeactivate_weapon_smart_car( entity weapon )
{
	weapon.StopWeaponEffect( $"P_smartpistol_lockon_FP", $"P_smartpistol_lockon" )
}

var function OnWeaponPrimaryAttack_weapon_smart_car( entity weapon, WeaponPrimaryAttackParams attackParams )
{
...
void function OnWeaponBulletHit_weapon_smart_car( entity weapon, WeaponBulletHitParams hitParams )
{
	//if ( weapon.HasMod( "proto_tracker" ) ) //Recheck for this once we make it mod only
	entity hitEnt = hitParams.hitEnt
	if ( IsValid( hitEnt ) )
	{
		weapon.SmartAmmo_TrackEntity( hitEnt, SMART_CAR_TRACKER_TIME )
			...
			if ( weapon.GetWeaponSettingBool( eWeaponVar.smart_ammo_player_targets_must_be_tracked ) )
			{
				if ( hitEnt.IsPlayer() &&  !hitEnt.IsTitan() ) //Note that there is a max of 10 status effects, which means that if you theoreteically get hit as a pilot 10 times without somehow dying, you could knock out other status effects like emp slow etc
					StatusEffect_AddTimed( hitEnt, eStatusEffect.lockon_detected, 1.0, SMART_CAR_TRACKER_TIME, 0.0 )
			}
		#endif
	}
...
void function OnWeaponStartZoomIn_weapon_smart_car( entity weapon )
{
...
void function OnWeaponStartZoomOut_weapon_smart_car( entity weapon )
{
	if ( weapon.HasMod( "ads_smaller_lock_on" ) )
	{
		array<string> mods = weapon.GetMods()
